...
namespace UnityEngine.GameGrowth
{
    public class GameGrowthLauncher : MonoBehaviour
    {
        ...
        public void DefaultAdjustAttributionCallback(AdjustAttribution adjustAttribution)
        {
            ...
            //Track attribution with Delta DNA. Unavailable data should be sent as empty string parameters
            var adjustAttributionEvent = new GameEvent(AdjustAttributionExtensions.adjustAttributionEventName)
                .AddParam(AdjustAttributionExtensions.acquisitionChannelParamName, adjustAttribution.network + "::" + adjustAttribution.campaign);
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.adGroupParamName, adjustAttribution.adgroup ?? string.Empty);
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.campaignParamName, adjustAttribution.campaign ?? string.Empty);
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.creativeParamName, adjustAttribution.creative ?? string.Empty);
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.networkParamName, adjustAttribution.network ?? string.Empty);
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.trackerNameParamName, adjustAttribution.trackerName ?? string.Empty);
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.trackerTokenParamName, adjustAttribution.trackerToken ?? string.Empty);
            // No value assigned for activityKind.
            adjustAttributionEvent.AddParam(AdjustAttributionExtensions.activityKind, string.Empty);
